...
{
	public static Set<string> PickSceneFiles(this string file_path)
	{
		var result = file_path.PickFilesThat(file =>
	...
	public static Set<string> PickFilesThat(this string file_path, Func<string, bool> filter)
	{
		var files = DirAccess.Open(file_path).GetFiles();
		var result = Set<string>.With(files.Length);

		foreach (var file in files)
			if (filter(file))
				result.Append(file_path.PathJoin(file));

		return result;
	}

	public static Set<string> FindSceneFiles(this string file_path, params string[] exclude_dirs)
	{
		var result = file_path.FindFilesThat(
